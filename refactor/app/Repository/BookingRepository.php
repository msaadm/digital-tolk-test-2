...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        $page_num = $page ?? "1"; // use Null coalescing
        $user = User::find($user_id);
        $emergency_jobs = array();
        if ($user && $user->is('customer')) {
            ...
            $user_type = 'customer';
            return ['emergency_jobs' => $emergency_jobs, 'normal_jobs' => [], 'jobs' => $jobs, 'user' => $user, 'user_type' => $user_type, 'num_pages' => 0, 'page_num' => 0];
        } elseif ($user && $user->is('translator')) {
            $jobs = Job::getTranslatorJobsHistoric($user->id, 'historic', $page_num); // remove extra variable job_ids
            $total_jobs = $jobs->total();
            $num_pages = ceil($total_jobs / 15);
            $user_type = 'translator';
            $normal_jobs = $jobs;
            return ['emergency_jobs' => $emergency_jobs, 'normal_jobs' => $normal_jobs, 'jobs' => $jobs, 'user' => $user, 'user_type' => $user_type, 'num_pages' => $num_pages, 'page_num' => $page_num];
        }
        // add default response
        return ['emergency_jobs' => $emergency_jobs, 'normal_jobs' => [], 'jobs' => [], 'user' => null, 'user_type' => null, 'num_pages' => 0, 'page_num' => 0];
    }
    ...
    public function store($user, $data)
    {
        ...
        $response = []; // create response array
        if ($user->user_type === env('CUSTOMER_ROLE_ID')) { // use ===
            // remove unnecessary self assignment

            if (!isset($data['from_language_id'])) {
                $response['message'] = "Du måste fylla in alla fält";
                $response['field_name'] = "from_language_id";
            } else if ($data['immediate'] === 'no') { // use ===
                if (isset($data['due_date']) && $data['due_date'] === '') { // use ===
                    $response['message'] = "Du måste fylla in alla fält";
                    $response['field_name'] = "due_date";
                } else if (isset($data['due_time']) && $data['due_time'] == '') {
                    $response['message'] = "Du måste fylla in alla fält";
                    $response['field_name'] = "due_time";
                } else if (!isset($data['customer_phone_type'], $data['customer_physical_type'])) {
            ...
            // Move all error response to separate response
            if (isset($response['message'])) {
                $response['status'] = 'fail';
                return $response;
            }

            // data should not be modified, instead create new
            ...
            $data['gender'] = 'none'; // define default value
            ...
            $response['id'] = $job->id;
            $data['job_for'] = $this->getJobData($job);
    ...
    public function storeJobEmail($data)
    {
        ...
        $name = $user->name; // move out common statement out of condition
        ...
        Event::fire(new JobCreated($job, $data, '*')); // Change Event Name
        return [
            'type' => $user_type,
            'job' => $job,
            'status' => 'success',
    ...
    public function jobToData($job)
    {
        ...
        $data['job_for'] = $this->getJobData($job, Lang::SV);
     ...
     */
    public function jobCompleted($post_data = array())
    {
        $job_id = $post_data["job_id"];
        $user_id = $post_data['user_id'];
        $job = Job::with('translatorJobRel')->find($job_id);

        $this->markJobAsCompleted($job, $user_id);
    }
    ...
    {
        $user = User::find($user_id);
        return $this->getPotentialJobs($user, ReturnType::OBJ);
    }
     ...
     */
    public function sendNotificationTranslator($job, $exclude_user_id, $data = [])
    {
        $users = User::all();
    ...
    public function updateJob($id, $data, $user)
    {
        ...
        if ($change_translator['translatorChanged']) $log_data[] = $change_translator['log_data'];
        ...
        $changeStatus = $this->changeStatus($job, $data, $change_translator['translatorChanged']);
        ...
        if ($job->due <= Carbon::now()) {
            ...
            $job->save();
            if ($change_due['dateChanged']) $this->sendChangedDateNotification($job, $old_time);
            if ($change_translator['translatorChanged']) $this->sendChangedTranslatorNotification($job, $current_translator, $change_translator['new_translator']);
            if ($lang_changed) $this->sendChangedLangNotification($job, $old_lang);
        }
    }
    ...
    private function changeStatus($job, $data, $changedTranslator)
    {
        ...
        if ($old_status != $data['status']) {
            switch ($job->status) {
                case 'timedout':
                    $status_changed = $this->changeTimedoutStatus($job, $data, $changedTranslator);
                    break;
                case 'completed':
                ...
                case 'started':
                    ...
                    break;
                case 'pending':
                    $status_changed = $this->changePendingStatus($job, $data, $changedTranslator);
                    break;
            ...
            if ($status_changed) {
                ...
                return ['status_changed' => true, 'log_data' => $log_data];
            }
        }
    }
     ...
     * @return mixed
     */
    private function getJobData($job, $lang = Lang::EN)
    {
        $data = array();
        if ($job->gender !== null) {
            if ($job->gender === 'male') {
        ...
        if ($job->certified != null) {
            if ($job->certified == 'both') {
                ...
                $data[] = $lang === Lang::SV ? 'Auktoriserad' : 'certified';
            } else if ($job->certified == 'n_health') {
                $data[] = $lang === Lang::SV ? 'Sjukvårdstolk' : 'Healthcare interpreter';
            } else if ($job->certified == 'law' || $job->certified == 'n_law') {
                $data[] = $lang === Lang::SV ? 'Rätttstolk' : 'Court interpreter';
            } else {
                $data[] = $job->certified;
            }
        }
    ...
    public function getPotentialJobs($user, $return_type = ReturnType::ID)
    {
        ...
        $user_languages = collect($languages)->pluck('lang_id')->all();
        ...
        $job_ids = Job::getJobs($user->id, $job_type, 'pending', $user_languages, $gender, $translator_level);
        foreach ($job_ids as $k => $job) {
            $job_user_id = $job->user_id;
            $check_town = Job::checkTowns($job_user_id, $user->id);

            if ($return_type === ReturnType::ID) {
    ...
    public function endJob($post_data)
    {
        ...
        $this->markJobAsCompleted($job, $user_id, true);
    ...
    public function getAll(Request $request, $limit = null)
    {
        ...
        if (isset($request_data['filter_timetype'])) {
            if ($request_data['filter_timetype'] == "created") {
            ...
            } else if ($request_data['filter_timetype'] == "due") {
                if (isset($request_data['from']) && $request_data['from'] != "") {
    ...
    public function alerts()
    {
        ...
        foreach ($jobs as $job) {
            ...
            if (count($sessionTime) >= 3) {
                $diff[$i] = (intval($sessionTime[0]) * 60) + intval($sessionTime[1]) + (intval($sessionTime[2]) / 60);
        ...
        if ($user && $user->is('superadmin')) {
            ...
            if (isset($request_data['lang']) && $request_data['lang'] != '') {
                $allJobs->whereIn('jobs.from_language_id', $request_data['lang'])
                    ->where('jobs.ignore', 0);
                /*$allJobs->where('jobs.from_language_id', '=', $request_data['lang']);*/
            }
            if (isset($request_data['status']) && $request_data['status'] != '') {
                $allJobs->whereIn('jobs.status', $request_data['status'])
                    ->where('jobs.ignore', 0);
                /*$allJobs->where('jobs.status', '=', $request_data['status']);*/
            }
            if (isset($request_data['customer_email']) && $request_data['customer_email'] != '') {
                $user = DB::table('users')->where('email', $request_data['customer_email'])->first();
                if ($user) {
            ...
            if (isset($request_data['filter_timetype']) && $request_data['filter_timetype'] == "created") {
                if (isset($request_data['from']) && $request_data['from'] != "") {
                    $allJobs->where('jobs.created_at', '>=', $request_data["from"])
                        ->where('jobs.ignore', 0);
                }
                if (isset($request_data['to']) && $request_data['to'] != "") {
                    $to = $request_data["to"] . " 23:59:00";
            ...
            if (isset($request_data['filter_timetype']) && $request_data['filter_timetype'] == "due") {
                if (isset($request_data['from']) && $request_data['from'] != "") {
                    $allJobs->where('jobs.due', '>=', $request_data["from"])
                        ->where('jobs.ignore', 0);
                }
                if (isset($request_data['to']) && $request_data['to'] != "") {
                    $to = $request_data["to"] . " 23:59:00";
            ...
            if (isset($request_data['job_type']) && $request_data['job_type'] != '') {
                $allJobs->whereIn('jobs.job_type', $request_data['job_type'])
                    ->where('jobs.ignore', 0);
                /*$allJobs->where('jobs.job_type', '=', $request_data['job_type']);*/
            }
        ...
        return ['allJobs' => $allJobs, 'languages' => $languages, 'all_customers' => $all_customers, 'all_translators' => $all_translators, 'requestdata' => $request_data];
    }
    ...
    private function getDateInterval($due_date)
    {
        $completed_date = date('Y-m-d H:i:s');
    ...
    private function markJobAsCompleted($job, $user_id, $update_interval = false)
    {
        $completed_date = date('Y-m-d H:i:s');
        $job->end_at = $completed_date;
        if ($update_interval) {
            $job->session_time = $this->getDateInterval($job->due);;
        }
